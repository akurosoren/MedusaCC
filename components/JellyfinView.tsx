...
const MediaCard: React.FC<{ item: JellyfinItem, imageUrl: string }> = ({ item, imageUrl }) => (
    <div className="glass-card flex-shrink-0 w-48 rounded-2xl overflow-hidden group relative border-white/[0.05]">
        ...
        <img src={imageUrl} alt={item.Name} className="w-full h-72 object-cover transition-transform duration-700 group-hover:scale-105" />
        <div className="absolute bottom-0 left-0 right-0 p-4 z-20">
            <h3 className="font-semibold text-sm truncate text-white tracking-tight">{item.Name}</h3>
            {item.SeriesName && <p className="text-[10px] text-gray-400 truncate mt-0.5 font-medium">{item.SeriesName}</p>}
        </div>
    </div>
);
...
const UpcomingCard: React.FC<{ item: SonarrCalendarItem, language: string }> = ({ item, language }) => {
    ...
    return (
         <div className="glass-card flex-shrink-0 w-52 rounded-2xl overflow-hidden group relative border-white/[0.05]">
             <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent z-10"></div>
            <img src={imageUrl} alt={item.series.title} className="w-full h-80 object-cover transition-transform duration-1000 group-hover:scale-110 opacity-70 group-hover:opacity-100" />
             <div className="absolute top-3 right-3 z-20 flex flex-col items-center bg-white/10 backdrop-blur-xl rounded-xl p-2 shadow-2xl border border-white/10">
                <span className="text-[9px] font-bold uppercase text-white/50 tracking-widest">{day}</span>
                <span className="text-xl font-bold text-white leading-none">{date.split('/')[0]}</span>
            </div>
            <div className="absolute bottom-0 left-0 right-0 p-5 z-20">
                <h3 className="font-semibold text-sm truncate text-white mb-1 tracking-tight" title={item.series.title}>{item.series.title}</h3>
                <div className="flex items-center space-x-2">
                    <span className="text-[9px] font-bold bg-white/10 px-2 py-0.5 rounded text-white/80 border border-white/5">S{item.seasonNumber} E{String(item.episodeNumber).padStart(2, '0')}</span>
                    <p className="text-[10px] text-gray-400 truncate flex-1 font-medium" title={item.title}>{item.title}</p>
                </div>
            </div>
        </div>
    );
...
const JellyfinView: React.FC = () => {
    ...
    useEffect(() => {
        const fetchJellyfinData = async () => {
            ...
            // Fetch Upcoming (requires Sonarr)
    ...
    return (
        <div className="w-full space-y-12 animate-fade-in-up pb-12">
            <div>
              <h1 className="text-6xl font-bold text-white tracking-tighter mb-2">{t('jellyfinTitle')}</h1>
              <p className="text-gray-400 text-lg font-medium">{t('jellyfinSubtitle')}</p>
            </div>
            ...
            <div className="space-y-16">
                ...
                {upcomingCalendar.length > 0 && (
                    <section>
                        <h2 className="text-3xl font-bold text-white mb-8 flex items-center tracking-tight">
                            <span className="w-1.5 h-8 bg-blue-500 rounded-full mr-5"></span>
                            {t('dashboardUpcoming')}
                        </h2>
                ...
                {latestMovies.length > 0 && (
                    <section>
                        <h2 className="text-3xl font-bold text-white mb-8 flex items-center tracking-tight">
                            <span className="w-1.5 h-8 bg-purple-500 rounded-full mr-5"></span>
                ...
                {latestSeries.length > 0 && (
                    <section>
                        <h2 className="text-3xl font-bold text-white mb-8 flex items-center tracking-tight">
                            <span className="w-1.5 h-8 bg-fuchsia-500 rounded-full mr-5"></span>
